...
import { ADMIN_TRIGGER_OPTIONS, type NotificationChannel } from '@/components/notification/types';
...
export const AdminNotificationPage = () => {
  ...
  if (isError) {
    return (
      <div>
        ...
        <ErrorBlock message={error?.message || '加载全局通知通道失败'} onRetry={() => refetch()} />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <Modal
        title={editingChannel ? '编辑全局通知通道' : '创建全局通知通道'}
        open={modalOpen}
        onOk={handleSubmit}
